...
public class CompressActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mTvFilePath = (ListView) findViewById(R.id.lv_compress_file);
        ...
        mCbShowPwd = (CheckBox) findViewById(R.id.cb_co_passwd_visib);
        ...
        if (path != null) {
            String[] paths = path.split("OtoDeleteFile:///");
        ...
        mTvFilePath.setAdapter(mCompressAdapter);
        if (mCompressList.size() > 5) {
            ViewGroup.LayoutParams params = mTvFilePath.getLayoutParams();
            params.height = 300;
            mTvFilePath.setLayoutParams(params);
        }
    ...
    private void compressProcess() {
        ...
        if (true) {
            if (mSpType.getSelectedItemPosition() == CompressUtils.TAR_GZ_POSITION
                ...
                cmd.append("'" + mEtDestination.getText() + File.separator +
                        ...
                        "' " + "'" + mEtDestination.getText() + File.separator + mEtFileName.getText() + ".tar" + "' ");
                mIsSpecialType = true;
            } else {
                sbCmd.append("-t" + mFileTypes[mSpType.getSelectedItemPosition()] + " ");    //7z a -t7z
                cmd = null;
                mIsSpecialType = false;
            }
        }
        sbCmd.append("'" + mEtDestination.getText() + File.separator + mEtFileName.getText() + "' "); //7z a '/hh.7z'
        sbCmd.append("'");
        ...
        new CompressUtils(CompressActivity.this, sbCmd.toString(), mIsSpecialType ? cmd.toString() : null, null).start();
        finish();
    }
